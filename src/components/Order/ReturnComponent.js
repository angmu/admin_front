...
export default class ReturnComponent extends Component {
  ...
  getContent = () => {
    ...
    let fData = [...this.props.changeData];
    if (!fData) return;
    ...
    return paginate(fData, currentPage, pageSize).map(data =>
      <tr key={data.key}>
        <td style={{ cursor: 'pointer', textDecoration: 'underline' }}
            onClick={() => {
              ...
              this.props.selectNum(
                this.props.orderListData.find((o) => o.key === data.key),
              );
            }}>{data.key}</td>
        <td>{data.pro_num}</td>
        {/*<td>{this.state.pdata ?*/}
        ...
        <td>{data.o_name}</td>
        <td><h2><Badge
          style={{ background: this.props.badgeColor(data.c_type), color: 'black' }}>{data.c_type}</Badge></h2></td>
        <td><h2><Badge
          style={{ background: this.props.badgeColor(data.c_type), color: 'black' }}>{data.c_state}</Badge></h2></td>
        <td>{data.c_reason}</td>
      </tr>,
    );
  };
